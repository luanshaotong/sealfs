...
impl MetaEngine {
    ...
    pub fn init(&self) {
        ...
        // }
        // self.file_indexs.insert("/".to_string(), 2);
    }
...
mod tests {
    ...
    fn test_create_delete_dir() {
        let db_path = "/tmp/test_dir_db";
        {
            ...
            let l = engine.file_indexs.get("/a").unwrap().clone();
            assert_eq!(2, l);
            let l = engine.file_indexs.get("/").unwrap().clone();
            assert_eq!(3, l);
            engine.directory_delete_entry("/", "a", 3).unwrap();
            engine.delete_directory("/a").unwrap();
            assert_eq!(engine.file_indexs.get("/a").is_none(), true);
            let l = engine.file_indexs.get("/").unwrap().clone();
            assert_eq!(2, l);
        }

        {
            ...
            let l = engine.file_indexs.get("/a1").unwrap().clone();
            assert_eq!(2, l);
            ...
            let l = engine.file_indexs.get("/a3").unwrap().clone();
            assert_eq!(2, l);
            engine.directory_delete_entry("/", "a3", 3).unwrap();
            engine.delete_directory("/a3").unwrap();
            assert_eq!(engine.file_indexs.get("/a3").is_none(), true);
